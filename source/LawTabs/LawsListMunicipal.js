...
export default function LawsListMunicipal() {
  const [ loadComplete, setLoading ] = useState(false);
  const [ laws, setLaws ] = useState([]);
  let [ lawsMunicipais, setLawsMunicipais ] = useState([]);
  ...
  async function getLawsFromApi() {
    ...
    try {
      ...
      setLawsMunicipais(
          laws.filter((law)=>{
              return law.typeLei == 'Municipal';
          })
      );
      console.log('laws municipais...');
      console.log(lawsMunicipais);
      setLoading(true);
    } catch (error) {
      console.log(error);
    }
